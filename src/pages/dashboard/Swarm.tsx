...
import { BROWSER_ALLOWLIST, MCP_TOOLS, OPERATOR_JOBS } from "@/lib/operatorRuntime";
...
export default function Swarm() {
  ...
  async function testProvider() {
    const validation = validateProviderConfig(provider);
    if (!validation.ok) {
      setProviderStatus(validation.error);
      return;
    }
    setProviderBusy(true);
    setProviderStatus("");
    try {
      const result = await probeProvider(validation.value);
      setProviderStatus(`connected · ${result.modelCount} models · ${result.endpoint}`);
    } catch (error) {
      setProviderStatus(error instanceof Error ? error.message : "Provider health check failed.");
    } finally {
      setProviderBusy(false);
    }
  }

  async function checkContentRewards() {
    setSourceBusy(true);
    setSourceStatus("");
    try {
      const response = await fetch(contentRewardsListUrl({ limit: 1, sortBy: "newest" }), { headers: { accept: "application/json" }, signal: AbortSignal.timeout(15_000) });
      if (!response.ok) throw new Error(`Discover HTTP ${response.status}`);
      const payload: unknown = await response.json();
      const data = payload && typeof payload === "object" && "data" in payload && Array.isArray((payload as { data?: unknown }).data) ? (payload as { data: unknown[] }).data.length : 0;
      setSourceStatus(`connected · public Discover · ${data} row returned`);
    } catch (error) {
      setSourceStatus(error instanceof Error ? error.message : "Content Rewards check failed.");
    } finally {
      setSourceBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1.6fr)]">
        <div className="min-w-0 space-y-6">
          ...
          <Panel title="Provider" meta="OpenAI-compatible">
            <div className="space-y-3">
              ...
              <div className="flex gap-2"><Button variant="outline" onClick={saveProvider} className="flex-1"><LockKeyhole className="h-3.5 w-3.5" /> Save config</Button><Button variant="outline" onClick={testProvider} disabled={providerBusy} className="flex-1"><CircleDot className="h-3.5 w-3.5" /> {providerBusy ? "Checking…" : "Test API"}</Button></div>
              {providerStatus && <p className="border border-primary/25 bg-primary/5 px-3 py-2 font-mono text-[10px] leading-relaxed text-primary">{providerStatus}</p>}
        ...
        <div className="min-w-0 space-y-6">
          ...
          <Panel title="Connector registry" meta="capability contract">
            <div className="space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-3 border border-primary/30 bg-primary/5 px-3 py-3"><div><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">public-api-check</p><p className="mt-1 text-[12px] text-muted-foreground">Content Rewards Discover JSON</p></div><Button size="sm" variant="outline" onClick={checkContentRewards} disabled={sourceBusy}>{sourceBusy ? "Checking…" : "Test API"}</Button></div>
              {sourceStatus && <p className="font-mono text-[10px] leading-relaxed text-primary">{sourceStatus}</p>}
          ...
          <Panel title="Operator services" meta="local process · official MCP SDK · read-only browser">
            <div className="space-y-3">
              <div className="grid gap-2 sm:grid-cols-2">
                <div className="border border-border/70 p-3"><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">sc</p><p className="mt-1 text-[12px] text-muted-foreground">CLI untuk source status, connector list, mcp list, dan daemon status.</p><code className="mt-2 block break-all text-[10px] text-foreground">bun run sc -- source status</code></div>
                <div className="border border-border/70 p-3"><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">sc:daemon</p><p className="mt-1 text-[12px] text-muted-foreground">Operator-run daemon dengan lock, state, interval, dan graceful shutdown.</p><code className="mt-2 block break-all text-[10px] text-foreground">bun run sc:daemon</code></div>
              </div>
              <div className="border border-border/70 p-3"><div className="flex items-center gap-2"><Terminal className="h-3.5 w-3.5 text-primary" /><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">MCP stdio tools</p></div><div className="mt-2 grid gap-1 sm:grid-cols-2">{MCP_TOOLS.map((tool) => <div key={tool.name} className="flex items-center justify-between gap-2 border-b border-border/50 py-1 text-[11px]"><span className="font-mono">{tool.name}</span><span className="text-muted-foreground">{tool.access}</span></div>)}</div><code className="mt-2 block break-all text-[10px] text-foreground">bun run sc:mcp</code></div>
              <div className="border border-border/70 p-3"><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">Safe browser runner</p><p className="mt-1 text-[12px] text-muted-foreground">Playwright built-in hanya membuka origin/path yang tercantum dan membaca title/text. Camofox menjadi contract opsional, bukan klaim stealth.</p><div className="mt-2 space-y-1 font-mono text-[10px] text-muted-foreground">{BROWSER_ALLOWLIST.map((entry) => <div key={entry.origin}>{entry.origin}{entry.paths.join(" · ")}</div>)}</div><code className="mt-2 block break-all text-[10px] text-foreground">bun run sc:browser -- https://contentrewards.com/discover</code></div>
              <div className="border border-border/70 p-3"><p className="font-mono text-[10px] uppercase tracking-[0.1em] text-primary">Autonomous jobs</p><div className="mt-2 space-y-2">{OPERATOR_JOBS.map((job) => <div key={job.id} className="flex items-start justify-between gap-3 text-[11px]"><span><span className="font-mono">{job.id}</span><span className="mt-0.5 block text-muted-foreground">{job.description}</span></span><span className="shrink-0 font-mono text-muted-foreground">{job.intervalSeconds}s</span></div>)}</div></div>
            </div>
          </Panel>

          <Panel title="Safety boundary" className="border-amber-500/25">
            <div className="flex gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-amber-300" /><ul className="space-y-2 text-[12px] leading-relaxed text-muted-foreground"><li>Agent tidak bisa menulis repo, memanggil shell, mengubah credential, atau memposting ke marketplace.</li><li>Memory dan transcript tersimpan lokal di browser; API key hanya sessionStorage.</li><li>Skill proposal, social publishing, dan tindakan consequential tetap membutuhkan approval manusia.</li><li>Browser cron hanya berjalan saat tab aktif; durable daemon harus dijalankan eksplisit dan tetap read-only.</li></ul></div>
          </Panel>
        </div>
      </div>
